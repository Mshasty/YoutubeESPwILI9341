...
#define CHANNEL_ID "UClVFdyV1cqWdplsWo9_1Aqw"               // makes up the url of channel
char CH_NAME[] = "PowerVoice.ru";
//--------------------------------------
...
void setup() {
  ...
  tft.drawBitmap(70, 90, wifiBitmap, 100, 70, ILI9341_WHITE);
  ...
  ui.drawString(120, 264, MY_IP);
  ui.drawString(120, 286, ipToString(WiFi.localIP()));
...
void loop() {
  if (millis() > api_lasttime + api_mtbs) {
    statYT();  
    Serial.print("Request to API");
  }
  if (millis() > ntp_lasttime + ntp_timer) {
    ntp(25, "pool.ntp.org", 3); // время NTP 
    ntp_lasttime = millis();
  }
